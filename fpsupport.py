...
from tg_bot import auto_response_cp, config_loader_cp, auto_delivery_cp, templates_cp, plugins_cp, file_uploader, \
    authorized_users_cp, proxy_cp, default_cp, multi_account_cp
...
def get_funpayautobot() -> None | funpayautobot:
    """
    Возвращает существующий экземпляр кардинала.
    """
    if hasattr(funpayautobot, "instance"):
        return getattr(funpayautobot, "instance")
...
class funpayautobot(object):
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "instance"):
            cls.instance = super(funpayautobot, cls).__new__(cls)
    ...
    def __init__(self, main_config: ConfigParser,
        ...
        # Прокси
        self.proxy = {}
        self.proxy_dict = FPManager.load_proxy_dict()  # прокси {0: "login:password@ip:port", 1: "ip:port"...}
        if self.MAIN_CFG["Proxy"].getboolean("enable"):
            if self.MAIN_CFG["Proxy"]["ip"] and self.MAIN_CFG["Proxy"]["port"].isnumeric():
                ...
                if proxy_str not in self.proxy_dict.values():
                    ...
                    FPManager.cache_proxy_dict(self.proxy_dict)

                if self.MAIN_CFG["Proxy"].getboolean("check") and not FPManager.check_proxy(self.proxy):
                    sys.exit()
        ...
        self.blacklist = FPManager.load_blacklist()  # ЧС.
        self.old_users = FPManager.load_old_users(
        ...
        self.disabled_plugins = FPManager.load_disabled_plugins()
        self.pinned_plugins = FPManager.load_pinned_plugins()
    ...
    def __init_account(self) -> None:
        ...
        while True:
            try:
                ...
                greeting_text = FPManager.create_greeting_text(self)
                FPManager.set_console_title("FunPay Support - bot")
    ...
    def raise_lots(self) -> int:
        ...
        for subcat in sorted(list(self.profile.get_sorted_lots(2).keys()), key=lambda x: x.category.position):
            ...
            try:
                ...
                time_delta = "" if not last_time else f" Последнее поднятие: {FPManager.time_to_str(new_time - last_time)} назад."
                time.sleep(1)
                self.account.raise_lots(subcat.category.id)
                ...
                if e.wait_time is not None:
                    logger.warning(_("crd_raise_time_err", subcat.category.name, error_text,
                                     FPManager.time_to_str(e.wait_time)))
    ...
    def parse_message_entities(self, msg_text: str) -> list[str | int | float]:
        ...
        pos = 0
        entities = []
        while entity := FPManager.ENTITY_RE.search(msg_text, pos=pos):
    ...
    def init(self):
        ...
        if self.MAIN_CFG["Telegram"].getboolean("enabled"):
            ...
            for module in [auto_response_cp, auto_delivery_cp, config_loader_cp, templates_cp, plugins_cp,
                           file_uploader, authorized_users_cp, proxy_cp, multi_account_cp, default_cp]:
    ...
    def toggle_plugin(self, uuid):
        ...
        FPManager.cache_disabled_plugins(self.disabled_plugins)
    ...
    def pin_plugin(self, uuid):
        ...
        FPManager.cache_pinned_plugins(self.pinned_plugins)
