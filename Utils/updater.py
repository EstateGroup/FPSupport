...
logger = getLogger("FunPayAutobot.update_checker")
localizer = Localizer()
...
CURRENT_SHA_FILE = os.path.join("storage", "cache", "current_commit")


def load_current_sha() -> str | None:
    if os.path.exists(CURRENT_SHA_FILE):
        with open(CURRENT_SHA_FILE, "r") as f:
            return f.read().strip() or None
    return None


def save_current_sha(sha: str):
    os.makedirs(os.path.dirname(CURRENT_SHA_FILE), exist_ok=True)
    with open(CURRENT_SHA_FILE, "w") as f:
        f.write(sha)
...
class Release:
    ...
    def __init__(self, name: str, description: str, sources_link: str, sha: str | None = None):
        """
        ...
        :param sha: полный SHA коммита (для main-based обновлений).
        """
        self.name = name
...
def get_tags(current_tag: str) -> list[str] | None:
    ...
    try:
        ...
        while not any([el.get("name") == current_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            response = requests.get(f"https://api.github.com/repos/EstateGroup/FPSupport/tags?page={page}",
                                    headers=HEADERS)
...
def get_releases(from_tag: str) -> list[Release] | None:
    ...
    try:
        ...
        while not any([el.get("tag_name") == from_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            response = requests.get(f"https://api.github.com/repos/EstateGroup/FPSupport/releases?page={page}",
                                    headers=HEADERS)
            if not response.status_code == 200 or not response.json():
                logger.debug(f"Update status code is {response.status_code}!")
                return None
...
def get_latest_from_main() -> Release | None:
    """
    Получает последний коммит из main ветки репозитория и формирует Release.
    Используется для репозиториев без тегов/релизов.

    :return: объект Release или None при ошибке.
    """
    try:
        response = requests.get(
            "https://api.github.com/repos/EstateGroup/FPSupport/commits/main",
            headers=HEADERS
        )
        if response.status_code != 200:
            logger.debug(f"Failed to get latest commit: {response.status_code}")
            return None
        data = response.json()
        sha_full = data.get("sha", "")
        sha_short = sha_full[:7]
        message = data.get("commit", {}).get("message", "Update")
        date = data.get("commit", {}).get("committer", {}).get("date", "")
        sources_link = "https://api.github.com/repos/EstateGroup/FPSupport/zipball/main"
        return Release(
            name=f"main ({sha_short})",
...
def get_new_releases(current_tag) -> int | list[Release]:
    ...
    if tags is None:
        # Если тегов нет, пробуем получить обновление напрямую из main
        release = get_latest_from_main()
        if release is None:
            return 1
        saved_sha = load_current_sha()
        if saved_sha and release.sha and saved_sha == release.sha:
            return 2
        return [release]
