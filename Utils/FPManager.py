...
logger = logging.getLogger("FunPayAutobot.FPSupport")
localizer = Localizer()
...
def create_greeting_text(funpayautobot: FPSupport):
    """
    Генерирует приветствие для вывода в консоль после загрузки данных о пользователе.
    """
    account = funpayautobot.account
    balance = funpayautobot.balance
...
# --- Мультиаккаунты ---
MULTI_ACCOUNTS_FILE = os.path.join("storage", "cache", "multi_accounts.json")


def load_multi_accounts() -> dict:
    if not os.path.exists(MULTI_ACCOUNTS_FILE):
        return {}
    try:
...
def save_multi_accounts(accounts: dict):
    os.makedirs(os.path.dirname(MULTI_ACCOUNTS_FILE), exist_ok=True)
    with open(MULTI_ACCOUNTS_FILE, "w", encoding="utf-8") as f:
        json.dump(accounts, f, ensure_ascii=False, indent=4)


def validate_proxy_format(proxy: str) -> bool:
    """Проверяет формат прокси login:password@ip:port"""
    import re
    pattern = r'^[^:]+:[^@]+@[^:]+:\d+$'
    return bool(re.match(pattern, proxy))


def check_proxy_health(proxy: str) -> tuple[bool, str]:
    """Проверяет работоспособность прокси. Возвращает (ok, message)."""
    import requests as req
    try:
        proxies = {"http": f"http://{proxy}", "https": f"http://{proxy}"}
        resp = req.get("https://api.ipify.org?format=json", proxies=proxies, timeout=10)
        if resp.status_code == 200:
            return True, resp.json().get("ip", "OK")
        return False, f"HTTP {resp.status_code}"
    except Exception as e:
        return False, str(e)


def save_account_stats(account_id: str, stats: dict):
    """Сохраняет статистику аккаунта."""
    stats_dir = os.path.join("storage", "multi_accounts", account_id)
...
def load_account_stats(account_id: str) -> dict:
    """Загружает статистику аккаунта."""
    stats_file = os.path.join("storage", "multi_accounts", account_id, "stats.json")
