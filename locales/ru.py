...
mm_proxy = "🌐 Прокси"
mm_multi_account = "👥 Мультиаккаунты"
...
access_granted = "🔓 Доступ к ПУ предоставлен!\n\n" \
                 ...
                 "⚙️ Чтобы открыть меню настроек <i>FPSupport</i>, введи команду /menu."
...
✅ <i>FPSupport</i> получает полную историю чата и видит все данные о всех новых сообщениях.
✅ <i>FPSupport</i> может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
✅ <i>FPSupport</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>FPSupport</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>FPSupport</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>FPSupport</i> увидит только последнее.
❌ <i>FPSupport</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>FPSupport</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>FPSupport</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>FPSupport</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>FPSupport</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>FPSupport</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
🔃 Как только <i>FPSupport</i> инициализируется, данное сообщение изменится.\n
📋 Если <i>FPSupport</i> долго не инициализируется, проверь логи с помощью команды /logs
...
FunPayAutobot_init = """✅ <b><u>FPSupport инициализирован!</u></b>\n
...
<b>ЦП:</b>
{}
    Используется <i>FPSupport</i>: <code>{}%</code>
...
update_lasted = "✅ У тебя стоит последняя версия FPSupport"
...
update_done = "✅ Установил обновление! Перезапусти <i>FPSupport</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>FPSupport.exe</code> находится в папке <code>update</code>. " \
                  "Выключи <i>FPSupport</i>, перенеси новый <code>FPSupport.exe</code> на место старого " \
...
pl_new = "Отправь мне плагин.\n\n<b>⚠️ ВНИМАНИЕ! Загрузка плагинов из сомнительных источников может привести к печальным последствиям.\n" \
         "@EstateMarket_robot решает большинство возможных проблем.</b>"

au_user_settings = "Настройки для пользователя {}"
adv_FunPayAutobot = "😎 FPSupport - лучший бот для FunPay"
...
desc_lang = desc_main
desc_gs = "Здесь ты можешь включать и выключать основные функции <i>FPSupport</i>."
...
cmd_restart = "перезапустить FPSupport"
cmd_power_off = "выключить FPSupport"
...
# Мультиаккаунты
ma_title = "👥 <b>Мультиаккаунты</b>"
ma_desc = "Здесь ты можешь управлять дополнительными аккаунтами FunPay."
ma_no_accounts = "Нет добавленных аккаунтов."
ma_account_info = """👤 <b>{}</b>
...
🔑 Golden Key: <code>{}...{}</code>
👑 Аккаунт: <code>{}</code> | <code>{}</code>
💰 Баланс: <code>{}₽, {}$, {}€</code>
📊 Активные заказы: <code>{}</code>
📦 Всего лотов: <code>{}</code>
🌐 Прокси: <code>{}</code>
📊 Статус: {}
🕐 Обновлено: <code>{}</code>"""

ma_status_running = "🟢 Работает"
ma_status_stopped = "🔴 Остановлен"
ma_status_proxy_error = "⚠️ Ошибка прокси"
ma_status_starting = "🔄 Запускается..."
ma_add_account = "➕ Добавить аккаунт"
ma_enter_golden_key = "🔑 Шаг 1/4: Введите golden_key от FunPay аккаунта:"
ma_enter_bot_token = "🤖 Шаг 2/4: Введите токен Telegram бота (получить у @BotFather):"
ma_enter_bot_password = "🔐 Шаг 3/4: Придумайте пароль для этого бота (мин. 8 символов):"
ma_enter_proxy = "🌐 Шаг 4/4: Введите прокси (ОБЯЗАТЕЛЬНО, формат: login:password@ip:port):"
ma_invalid_golden_key = "❌ Неверный формат golden_key. Должен быть 32 символа."
ma_invalid_bot_token = "❌ Неверный формат токена бота. Попробуйте еще раз."
ma_invalid_password = "❌ Слабый пароль. Мин. 8 символов, заглавные + строчные + цифра."
ma_invalid_proxy = "❌ Неверный формат прокси. Используйте: login:password@ip:port"
ma_proxy_required = "⚠️ Прокси обязателен для мультиаккаунтов."
ma_account_added = "✅ Мультиаккаунт создан!\n🔑 golden_key: <code>{}</code>\n🤖 Бот: @{}\n\nТеперь напишите /start этому боту для начала работы."
ma_account_started = "✅ Аккаунт <b>{}</b> запущен."
ma_account_stopped = "✅ Аккаунт <b>{}</b> остановлен."
ma_account_deleted = "✅ Аккаунт <b>{}</b> удалён."
ma_account_not_found = "❌ Аккаунт не найден."
ma_proxy_updated = "✅ Прокси для аккаунта <b>{}</b> обновлён."
ma_proxy_check_fail = "❌ Прокси не прошёл проверку: {}"
ma_confirm_delete = "Вы уверены, что хотите удалить аккаунт <b>{}</b>?"
ma_back = "🔙 Назад к аккаунтам"
ma_start = "▶️ Запустить"
ma_stop = "⏹ Остановить"
ma_edit_proxy = "🌐 Изменить прокси"
ma_delete = "🗑 Удалить"
ma_confirm_yes = "✅ Да, удалить"
ma_confirm_no = "❌ Отмена"
