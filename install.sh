...
echo -e "\n\n${RED} * GitHub ${CYAN}github.com/EstateGroup/FPManager${RESET}"
echo -e "${RED} * Telegram ${CYAN}t.me/funpay_funpayautobot${RESET}"
echo -e "\n\n\n"


echo -ne "${CYAN}Введите имя пользователя, от имени которого будет запускаться бот (например, 'funpayautobot'): ${RESET}"
while true; do
  read username
...
clear
echo -e "$start_process_line\nУстанавливаю FPManager...\n$end_process_line"


#13
if ! sudo mkdir /home/$username/FunPayAutobot-install ; then
  echo -e "${start_process_line}\nПроизошла ошибка при создании директории для установки. (13/${commands})\n${end_process_line}"
  exit 2
fi

gh_repo="EstateGroup/FPManager"
LOCATION=$(curl -sS https://api.github.com/repos/$gh_repo/releases/latest | grep "zipball_url" | awk '{ print $2 }' | sed 's/,$//' | sed 's/"//g' )

#14
if ! sudo curl -L $LOCATION -o /home/$username/FunPayAutobot-install/FunPayAutobot.zip ; then
  echo -e "${start_process_line}\nПроизошла ошибка при загрузке архива. (14/${commands})\n${end_process_line}"
  exit 2
fi

#15
if ! sudo unzip /home/$username/FunPayAutobot-install/FunPayAutobot.zip -d /home/$username/FunPayAutobot-install ; then
  echo -e "${start_process_line}\nПроизошла ошибка при распаковке архива. (15/${commands})\n${end_process_line}"
  exit 2
fi

#16
if ! sudo mkdir /home/$username/FPManager ; then
  echo -e "${start_process_line}\nПроизошла ошибка при создании директории для бота. (16/${commands})\n${end_process_line}"
  exit 2
fi

#17
if ! sudo bash -c "mv /home/$username/FunPayAutobot-install/*/* /home/$username/FPManager/"; then
  echo -e "${start_process_line}\nПроизошла ошибка при перемещении файлов. (17/${commands})\n${end_process_line}"
  exit 2
fi

#18
if ! sudo rm -rf /home/$username/FunPayAutobot-install ; then
  echo -e "${start_process_line}\nПроизошла ошибка при удалении директории для установки. (18/${commands})\n${end_process_line}"
  exit 2
fi

#19
if ! sudo chown -hR $username:$username /home/$username/FPManager ; then
  echo -e "${start_process_line}\nПроизошла ошибка при изменении владельца файлов. (19/${commands})\n${end_process_line}"
  exit 2
fi

#20
if ! sudo -u $username /home/$username/pyvenv/bin/pip install -U -r /home/$username/FPManager/requirements.txt ; then
...
#21
if ! sudo ln -sf /home/$username/FPManager/FPManager@.service /etc/systemd/system/FPManager@.service ; then
...
clear
echo -e '\n\n\e[1;91m * GitHub \e[1;96mgithub.com/EstateGroup/FPManager\e[0m'
echo -e '\e[1;91m * Telegram \e[1;96mt.me/funpay_funpayautobot\e[0m'
...
sudo -u $username LANG=en_US.utf8 /home/$username/pyvenv/bin/python /home/$username/FPManager/main.py <&1
sudo systemctl start FPManager@$username.service
...
echo -e "\nГотово!"
echo -e "FPManager запущен как фоновый процесс!"
echo -e "Теперь напиши своему Telegram-боту."
echo -e "\n\e[1;92mДля остановки FPManager используй команду \e[93msudo systemctl stop FPManager@${username}\e[1;92m"
echo -e "Для запуска FPManager используй команду \e[93msudo systemctl start FPManager@${username}\e[1;92m"
echo -e "Для перезапуска FPManager используй команду \e[93msudo systemctl restart FPManager@${username}\e[1;92m"
echo -e "Для просмотра логов используй команду \e[93msudo systemctl status FPManager@${username} -n100\e[1;92m"
echo -e "Для добавления FPManager в автозагрузку используй команду \e[93msudo systemctl enable FPManager@${username}\e[1;92m"
echo -e "${RED}* Перед добавлением FPManager в автозагрузку убедись, что твой бот работает корректно.\e[1;92m"
echo -e "################################################################################\e[0m"
