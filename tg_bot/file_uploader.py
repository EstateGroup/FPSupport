...
from Utils import config_loader as cfg_loader, exceptions as excs, FPManager, updater
...
def init_uploader(funpayautobot: FPSupport):
    ...
    def upload_products_file(m: types.Message):
        ...
        try:
            products_count = FPManager.count_products(f"storage/products/{utils.escape(m.document.file_name)}")
        except:
    ...
    def upload_main_config(m: types.Message):
        ...
        funpayautobot.save_config(new_config, "configs/_main.cfg")
        logger.info(f"Пользователь $MAGENTA@{m.from_user.username} (id: {m.from_user.id})$RESET "
    ...
    def upload_auto_response_config(m: types.Message):
        ...
        funpayautobot.RAW_AR_CFG, funpayautobot.AR_CFG = raw_new_config, new_config
    ...
    def upload_auto_delivery_config(m: types.Message):
        ...
        funpayautobot.AD_CFG = new_config
        funpayautobot.save_config(funpayautobot.AD_CFG, "configs/auto_delivery.cfg")
    ...
    def upload_plugin(m: types.Message):
        ...
        bot.send_message(m.chat.id,
                         ...
                         f"⚠️Чтобы плагин активировался, <u><b>перезагрузите FPManager!</b></u> (/restart)",
                         reply_markup=keyboard)
    ...
    def send_funpay_image(m: types.Message):
        ...
        try:
            ...
            file = tg.bot.download_file(file_info.file_path)
            image_id = funpayautobot.account.upload_image(file, type_="chat")
            result = funpayautobot.send_message(chat_id, f"$photo={image_id}", username, watermark=False)
            if not result:
    ...
    def upload_image(m: types.Message, type_: Literal["chat", "offer"] = "chat"):
        ...
        try:
            file_info = tg.bot.get_file(photo.file_id)
            file = tg.bot.download_file(file_info.file_path)
            image_id = funpayautobot.account.upload_image(file, type_=type_)
        except:
