...
def init_auto_delivery_cp(crd: FPSupport, *args):
    tg = crd.telegram
    bot = tg.bot
    ...
    def add_lot_manually(m: Message):
        ...
        crd.AD_CFG.set(lot, "response", """💎 Привет, $username!

✅ Спасибо за покупку $order_title | $category!
🔗$product
⚠️ Не забудьте подтвердить заказ!
🛎 Дата покупки - $date""")
    ...
    def add_ad_to_lot(c: CallbackQuery):
        ...
        crd.AD_CFG.set(lot.title, "response", "💎 Привет, $username!\n\n✅ Спасибо за покупку $order_title | $category!\n🔗$product\n⚠️ Не забудьте подтвердить заказ!\n🛎 Дата покупки - $date")
        crd.save_config(crd.AD_CFG, "configs/auto_delivery.cfg")
    ...
    def open_gf_settings(c: CallbackQuery):
        ...
        products_amount = FPManager.count_products(f"storage/products/{file_name}")
        nl = "\n"
