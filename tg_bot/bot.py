...
from Utils import FPManager, updater
...
class TGBot:
    def __init__(self, funpayautobot: FPSupport):
        self.funpayautobot = funpayautobot
        self.bot = telebot.TeleBot(self.funpayautobot.MAIN_CFG["Telegram"]["token"], parse_mode="HTML",
    ...
    def safe_edit_text(self, text: str, chat_id: int, message_id: int, reply_markup=None, **kwargs):
        """
        Безопасно редактирует сообщение, обрабатывая переход от фото к тексту.
        ...
        """
        try:
            self.bot.edit_message_text(text, chat_id, message_id, reply_markup=reply_markup, **kwargs)
        except ApiTelegramException:
            try:
                self.bot.delete_message(chat_id, message_id)
            except:
                pass
            self.bot.send_message(chat_id, text, reply_markup=reply_markup, **kwargs)
    ...
    def reg_admin(self, m: Message):
        ...
        if m.chat.type != "private" or (self.attempts.get(m.from_user.id, 0) >= 5) or m.text is None:
            return
        if not self.funpayautobot.block_tg_login and \
                FPManager.check_password(m.text, self.funpayautobot.MAIN_CFG["Telegram"]["secretKeyHash"]):
    ...
    def ignore_unauthorized_users(self, c: CallbackQuery):
        ...
        if self.attempts[c.from_user.id] <= 5:
            self.bot.answer_callback_query(c.id, _("adv_FunPayAutobot", language=c.from_user.language_code), show_alert=True)
        return

    # Команды
    def send_settings_menu(self, m: Message):
        """
        Отправляет основное меню настроек.
        """
    ...
    def send_profile(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, utils.generate_profile_text(self.funpayautobot),
    ...
    def change_cookie(self, m: Message):
        ...
        new_account = Account(golden_key, self.funpayautobot.account.user_agent, proxy=self.funpayautobot.proxy,
                              locale=self.funpayautobot.account.locale)
        try:
            new_account.get()
        except:
        ...
        one_acc = False
        if new_account.id == self.funpayautobot.account.id or self.funpayautobot.account.id is None:
            one_acc = True
            self.funpayautobot.account.golden_key = golden_key
            try:
                self.funpayautobot.account.get()
            except:
                logger.warning("Произошла ошибка")  # locale
                logger.debug("TRACEBACK", exc_info=True)
                self.bot.send_message(m.chat.id, _("cookie_error"))
            ...
            accs = f" (<a href='https://funpay.com/users/{self.funpayautobot.account.id}/'>" \
                   f"{self.funpayautobot.account.username}</a> ➔ <a href='https://funpay.com/users/{new_account.id}/'>" \
                   f"{new_account.username}</a>)"

        self.funpayautobot.MAIN_CFG.set("FunPay", "golden_key", golden_key)
        self.funpayautobot.save_config(self.funpayautobot.MAIN_CFG, "configs/_main.cfg")
    ...
    def update_profile(self, c: CallbackQuery):
        ...
        try:
            self.funpayautobot.account.get()
            self.funpayautobot.balance = self.funpayautobot.get_balance()
        except:
        ...
        self.bot.edit_message_text(utils.generate_profile_text(self.funpayautobot), c.message.chat.id,
    ...
    def manual_delivery_text(self, m: Message):
        ...
        self.funpayautobot.delivery_tests[key] = lot_name
    ...
    def ban(self, m: Message):
        ...
        if nickname in self.funpayautobot.blacklist:
            self.bot.send_message(m.chat.id, _("already_blacklisted", nickname))
            return

        self.funpayautobot.blacklist.append(nickname)
        FPManager.cache_blacklist(self.funpayautobot.blacklist)
    ...
    def unban(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        nickname = m.text.strip()
        if nickname not in self.funpayautobot.blacklist:
            self.bot.send_message(m.chat.id, _("not_blacklisted", nickname))
            return
        self.funpayautobot.blacklist.remove(nickname)
        FPManager.cache_blacklist(self.funpayautobot.blacklist)
    ...
    def send_ban_list(self, m: Message):
        ...
        Отправляет ЧС.
        """
        if not self.funpayautobot.blacklist:
            self.bot.send_message(m.chat.id, _("blacklist_empty"))
            return
        blacklist = ", ".join(f"<code>{i}</code>" for i in sorted(self.funpayautobot.blacklist, key=lambda x: x.lower()))
    ...
    def act_edit_watermark(self, m: Message):
        ...
        """
        watermark = self.funpayautobot.MAIN_CFG["Other"]["watermark"]
    ...
    def edit_watermark(self, m: Message):
        ...
        self.funpayautobot.MAIN_CFG["Other"]["watermark"] = watermark
        self.funpayautobot.save_config(self.funpayautobot.MAIN_CFG, "configs/_main.cfg")
        if watermark:
            ...
            try:
                with open("logs/log.log", "r", encoding="utf-8") as f:
                    self.bot.send_document(m.chat.id, f,
                                           caption=f'{_("gs_old_msg_mode").replace("{} ", "") if self.funpayautobot.old_mode_enabled else ""}')
                    f.seek(0)
                    file_content = f.read()
    ...
    def about(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("about"))

    def check_updates(self, m: Message):
        releases = updater.get_new_releases(None)
    ...
    def update(self, m: Message):
        ...
        if release.sha:
            updater.save_current_sha(release.sha)
    ...
    def send_system_info(self, m: Message):
        ...
        uptime = current_time - self.funpayautobot.start_time
        ...
        self.bot.send_message(m.chat.id, _("sys_info", cpu_usage, psutil.Process().cpu_percent(),
                                           ...
                                           FPManager.time_to_str(uptime), m.chat.id))

    def restart_funpayautobot(self, m: Message):
        """
        Перезапускает кардинал.
        """
        self.bot.send_message(m.chat.id, _("restarting"))
        FPManager.restart_program()

    def ask_power_off(self, m: Message):
        """
        Просит подтверждение на отключение FPManager.
        """
        self.bot.send_message(m.chat.id, _("power_off_0"), reply_markup=kb.power_off(self.funpayautobot.instance_id, 0))
    ...
    def power_off(self, c: CallbackQuery):
        ...
        if instance_id != self.funpayautobot.instance_id:
    ...
    def send_funpay_message(self, message: Message):
        ...
        result = self.funpayautobot.send_message(node_id, response_text, username, watermark=False)
        if result:
    ...
    def edit_greetings_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.funpayautobot.MAIN_CFG["Greetings"]["greetingsText"] = m.text
    ...
    def edit_greetings_cooldown(self, m: Message):
        ...
        try:
            ...
            return
        self.funpayautobot.MAIN_CFG["Greetings"]["greetingsCooldown"] = str(cooldown)
    ...
    def edit_order_confirm_reply_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.funpayautobot.MAIN_CFG["OrderConfirm"]["replyText"] = m.text
    ...
    def edit_review_reply_text(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.funpayautobot.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"] = m.text
    ...
    def extend_new_message_notification(self, c: CallbackQuery):
        ...
        try:
            chat = self.funpayautobot.account.get_chat(int(chat_id))
        except:
            self.bot.answer_callback_query(c.id)
        ...
        for i in messages:
            if i.author_id == last_message_author_id and i.by_bot == last_by_bot and i.badge == last_badge and \
                ...
                author = ""
            elif i.author_id == self.funpayautobot.account.id:
                author = f"<i><b>🤖 {_('you')} (<i>FPSupport</i>):</b></i> " if i.by_bot else f"<i><b>🫵 {_('you')}:</b></i> "
                if i.is_autoreply:
                ...
                elif i.author in self.funpayautobot.blacklist:
                    author = f"<i><b>🚷 {i.author}: </b></i>"
                elif i.by_bot:
                    author = f"<i><b>🐦 {i.author}: </b></i>"
                elif i.by_vertex:
            ...
            msg_text = f"<code>{utils.escape(i.text)}</code>" if i.text else \
                ...
                f"{self.funpayautobot.show_image_name and not (i.author_id == self.funpayautobot.account.id and i.by_bot) and i.image_name or _('photo')}</a>"
    ...
    def refund(self, c: CallbackQuery):
        ...
        while attempts:
            try:
                self.funpayautobot.account.refund(order_id)
                break
            except:
                if not new_msg:
    ...
    def open_cp(self, c: CallbackQuery):
        ...
        try:
            ...
            pass
        self._edit_settings_message(c, _("desc_main"), skb.SETTINGS_SECTIONS())
    ...
    def open_cp2(self, c: CallbackQuery):
        ...
        try:
            ...
            pass
        self._edit_settings_message(c, _("desc_main"), skb.SETTINGS_SECTIONS_2())

    def _edit_settings_message(self, c: CallbackQuery, text: str, reply_markup):
        """
        Редактирует сообщение настроек.
        """
        try:
            self.bot.edit_message_text(text, c.message.chat.id, c.message.id,
            ...
            try:
                ...
                pass
            self.bot.send_message(c.message.chat.id, text, reply_markup=reply_markup, parse_mode="HTML")

    def switch_param(self, c: CallbackQuery):
        """
        Переключает переключаемые настройки FPManager.
        """
        split = c.data.split(":")
        section, option = split[1], split[2]
        if section == "FunPay" and option == "oldMsgGetMode":
            self.funpayautobot.switch_msg_get_mode()
        else:
            self.funpayautobot.MAIN_CFG[section][option] = str(int(not int(self.funpayautobot.MAIN_CFG[section][option])))
            self.funpayautobot.save_config(self.funpayautobot.MAIN_CFG, "configs/_main.cfg")
        ...
        if section == "Telegram":
            self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                               reply_markup=kb.authorized_users(self.funpayautobot, offset=int(split[3])))
        else:
            self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                               reply_markup=sections[section](self.funpayautobot))
        logger.info(_("log_param_changed", c.from_user.username, c.from_user.id, option, section,
                      self.funpayautobot.MAIN_CFG[section][option]))
        self.bot.answer_callback_query(c.id)
    ...
    def switch_chat_notification(self, c: CallbackQuery):
        ...
        self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                           reply_markup=keyboard(self.funpayautobot, c.message.chat.id))
        self.bot.answer_callback_query(c.id)
    ...
    def open_settings_section(self, c: CallbackQuery):
        ...
        sections = {
            "lang": (_("desc_lang"), kb.language_settings, [self.funpayautobot]),
            "main": (_("desc_gs"), kb.main_settings, [self.funpayautobot]),
            "tg": (_("desc_ns", c.message.chat.id), kb.notifications_settings, [self.funpayautobot, c.message.chat.id]),
            "bl": (_("desc_bl"), kb.blacklist_settings, [self.funpayautobot]),
            "ar": (_("desc_ar"), skb.AR_SETTINGS, []),
            "ad": (_("desc_ad"), skb.AD_SETTINGS, []),
            "mv": (_("desc_mv"), kb.new_message_view_settings, [self.funpayautobot]),
            "rr": (_("desc_or"), kb.review_reply_settings, [self.funpayautobot]),
            "gr": (_("desc_gr", utils.escape(self.funpayautobot.MAIN_CFG['Greetings']['greetingsText'])),
                   kb.greeting_settings, [self.funpayautobot]),
            "oc": (_("desc_oc", utils.escape(self.funpayautobot.MAIN_CFG['OrderConfirm']['replyText'])),
                   kb.order_confirm_reply_settings, [self.funpayautobot])
        }

        curr = sections[section]
        reply_markup = curr[1](*curr[2])
        try:
            self.bot.edit_message_text(curr[0], c.message.chat.id, c.message.id,
                                       reply_markup=reply_markup, parse_mode="HTML")
        except ApiTelegramException:
            pass
        self.bot.answer_callback_query(c.id)
    ...
    def send_announcements_kb(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("desc_an"), reply_markup=kb.announcements_settings(self.funpayautobot, m.chat.id))

    def send_review_reply_text(self, c: CallbackQuery):
        stars = int(c.data.split(":")[1])
        text = self.funpayautobot.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"]
        keyboard = K() \
            .row(B(_("gl_back"), callback_data=f"{CBT.CATEGORY}:rr"),
                 B(_("gl_edit"), callback_data=f"{CBT.EDIT_REVIEW_REPLY_TEXT}:{stars}"))
        if not text:
            ...
            self.bot.send_message(c.message.chat.id, _("review_reply_text", "⭐" * stars,
                                                       self.funpayautobot.MAIN_CFG['ReviewReply'][f'star{stars}ReplyText']),
    ...
    def empty_callback(self, c: CallbackQuery):
        self.bot.answer_callback_query(c.id, "🤖 FPSupport 🤖")

    def switch_lang(self, c: CallbackQuery):
        lang = c.data.split(":")[1]
        Localizer(lang)
        self.funpayautobot.MAIN_CFG["Other"]["language"] = lang
        self.funpayautobot.save_config(self.funpayautobot.MAIN_CFG, "configs/_main.cfg")
        if localizer.current_language == "en":
            self.bot.answer_callback_query(c.id, "Language changed to English.", show_alert=True)
        elif localizer.current_language == "ru":
            self.bot.answer_callback_query(c.id, "Язык изменен на русский.", show_alert=True)
    ...
    def __register_handlers(self):
        ...
        self.msg_handler(self.restart_funpayautobot, commands=["restart"])
    ...
    def edit_bot(self):
        ...
        """
        try:
            default_name = "FunPay Support - bot | Фанпей Авто Выдача"
            ...
            logger.debug(f"edit_bot: ошибка при изменении имени бота: {e}")

        try:
            sh_text = "🛠️ github.com/EstateGroup/FPSupport\n👨‍💻 @EstateGroup0\n🧩 @EstateMarket_robot\n💬 @funpay_supp\n📰 @estatemarket_news\n🛡 @Estate_Support_bot"
            res = self.bot.get_my_short_description().short_description
            if res != sh_text:
                self.bot.set_my_short_description(sh_text)
        except Exception as e:
            logger.debug(f"edit_bot: ошибка при изменении краткого описания: {e}")

        try:
            for i in [None, *localizer.languages.keys()]:
                res = self.bot.get_my_description(i).description
                text = _("adv_description", language=i)
                if res != text:
                    self.bot.set_my_description(text, language_code=i)
        except Exception as e:
            logger.debug(f"edit_bot: ошибка при изменении описания: {e}")
