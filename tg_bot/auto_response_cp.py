...
def init_auto_response_cp(funpayautobot: FPSupport, *args):
    tg = funpayautobot.telegram
    bot = tg.bot
    ...
    def check_command_exists(command_index: int, message_obj: Message, reply_mode: bool = True) -> bool:
        ...
        """
        if command_index > len(funpayautobot.RAW_AR_CFG.sections()) - 1:
    ...
    def open_commands_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_ar_list"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.commands_list(funpayautobot, offset))
    ...
    def add_command(m: Message):
        ...
        for cmd in commands:
            if commands.count(cmd) > 1:
                ...
                return
            if cmd in funpayautobot.AR_CFG.sections():
                bot.reply_to(m, _("ar_cmd_already_exists_err", utils.escape(cmd)), reply_markup=error_keyboard)
                return

        funpayautobot.RAW_AR_CFG.add_section(raw_command)
        funpayautobot.RAW_AR_CFG.set(raw_command, "response", "Данной команде необходимо настроить текст ответа :(")
        funpayautobot.RAW_AR_CFG.set(raw_command, "telegramNotification", "0")
        funpayautobot.RAW_AR_CFG.set(raw_command, "enabled", "1")

        for cmd in commands:
            funpayautobot.AR_CFG.add_section(cmd)
            funpayautobot.AR_CFG.set(cmd, "response", "Данной команде необходимо настроить текст ответа :(")
            funpayautobot.AR_CFG.set(cmd, "telegramNotification", "0")
            funpayautobot.AR_CFG.set(cmd, "enabled", "1")

        funpayautobot.save_config(funpayautobot.RAW_AR_CFG, "configs/auto_response.cfg")

        command_index = len(funpayautobot.RAW_AR_CFG.sections()) - 1
    ...
    def open_edit_command_cp(c: CallbackQuery):
        ...
        keyboard = keyboards.edit_command(funpayautobot, command_index, offset)

        command = funpayautobot.RAW_AR_CFG.sections()[command_index]
        command_obj = funpayautobot.RAW_AR_CFG[command]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
    ...
    def edit_command_response(m: Message):
        ...
        funpayautobot.RAW_AR_CFG.set(command, "response", response_text)
        for cmd in commands:
            funpayautobot.AR_CFG.set(cmd, "response", response_text)
        funpayautobot.save_config(funpayautobot.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def edit_command_notification(m: Message):
        ...
        command = funpayautobot.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        funpayautobot.RAW_AR_CFG.set(command, "notificationText", notification_text)

        for cmd in commands:
            funpayautobot.AR_CFG.set(cmd, "notificationText", notification_text)
        funpayautobot.save_config(funpayautobot.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def switch_command_settings(c: CallbackQuery):
        ...
        if command_obj.get(setting) in [None, "0"]:
            ...
            value = "0"
        funpayautobot.RAW_AR_CFG.set(command, setting, value)
        for cmd in commands:
            funpayautobot.AR_CFG.set(cmd, setting, value)
        funpayautobot.save_config(funpayautobot.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def del_command(c: CallbackQuery):
        ...
        command = funpayautobot.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        funpayautobot.RAW_AR_CFG.remove_section(command)
        for cmd in commands:
            funpayautobot.AR_CFG.remove_section(cmd)
        funpayautobot.save_config(funpayautobot.RAW_AR_CFG, "configs/auto_response.cfg")
