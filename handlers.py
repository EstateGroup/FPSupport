...
logger = logging.getLogger("FunPayAutobot.handlers")
localizer = Localizer()
...
# INIT MESSAGE
def save_init_chats_handler(c: FPSupport, e: InitialChatEvent):
    """
    ...
    if c.MAIN_CFG["Greetings"].getboolean("sendGreetings") and e.chat.id not in c.old_users:
        ...
        FPManager.cache_old_users(c.old_users)


def update_threshold_on_initial_chat(c: FPSupport, e: InitialChatEvent):
    """
...
def old_log_msg_handler(c: FPSupport, e: LastChatMessageChangedEvent):
    """
...
def log_msg_handler(c: FPSupport, e: NewMessageEvent):
...
def update_threshold_on_last_message_change(c: FPSupport, e: LastChatMessageChangedEvent | NewMessageEvent):
    """
    Обновляет пороговое значение для определения новых чатов.
    """
...
def greetings_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    text = FPManager.format_msg_text(c.MAIN_CFG["Greetings"]["greetingsText"], obj)
    Thread(target=c.send_message, args=(chat_id, text, chat_name), daemon=True).start()


def add_old_user_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_response_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    response_text = FPManager.format_msg_text(c.AR_CFG[command]["response"], obj)
    Thread(target=c.send_message, args=(chat_id, response_text, chat_name), daemon=True).start()


def old_send_new_msg_notification_handler(c: FPSupport, e: LastChatMessageChangedEvent):
...
def send_new_msg_notification_handler(c: FPSupport, e: NewMessageEvent) -> None:
    """
    ...
    for i in events:
        ...
        if i.message.author_id == last_message_author_id and i.message.by_bot == last_by_bot and \
            ...
            author = f"<i><b>🤖 {_('you')} (<i>FPManager</i>):</b></i> " if i.message.by_bot else f"<i><b>🫵 {_('you')}:</b></i> "
...
def send_review_notification(c: FPSupport, order: Order, chat_id: int, reply_text: str | None):
    if not c.telegram:
        return
...
def process_review_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
        obj = e.message
    ...
    def send_reply():
        ...
        if c.MAIN_CFG["ReviewReply"].getboolean(toggle) and c.MAIN_CFG["ReviewReply"].get(text):
            try:
                ...
                reply_text = FPManager.format_order_text(c.MAIN_CFG["ReviewReply"].get(text), order)
...
def send_command_notification_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    if not c.AR_CFG[command].get("notificationText"):
        ...
        text = FPManager.format_msg_text(c.AR_CFG[command]["notificationText"], obj)
...
def test_auto_delivery_handler(c: FPSupport, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_categories_raised_notification_handler(c: FPSupport, cat: types.Category, error_text: str = "") -> None:
    """
...
def get_lot_config_by_name(c: FPSupport, name: str) -> configparser.SectionProxy | None:
    """
...
def check_products_amount(config_obj: configparser.SectionProxy) -> int:
    ...
    if not file_name:
        return 1
    return FPManager.count_products(f"storage/products/{file_name}")


# Новый ордер (REGISTER_TO_NEW_ORDER)
def log_new_order_handler(c: FPSupport, e: NewOrderEvent, *args):
    """
...
def setup_event_attributes_handler(c: FPSupport, e: NewOrderEvent, *args):
...
def send_new_order_notification_handler(c: FPSupport, e: NewOrderEvent, *args):
    """
...
def deliver_goods(c: FPSupport, e: NewOrderEvent, *args):
    chat = c.account.get_chat_by_name(e.order.buyer_username)
    if chat:
        chat_id = chat.id
    else:
    ...
    delivery_text = FPManager.format_order_text(cfg_obj["response"], e.order)
    ...
    try:
        if file_name := cfg_obj.get("productsFileName"):
            ...
            products, goods_left = FPManager.get_products(f"storage/products/{file_name}", amount)
    ...
    if not result:
        ...
        if file_name and products:
            FPManager.add_products(f"storage/products/{file_name}", products, at_zero_position=True)
    else:
...
def deliver_product_handler(c: FPSupport, e: NewOrderEvent, *args) -> None:
    """
...
def send_delivery_notification_handler(c: FPSupport, e: NewOrderEvent):
    """
...
def update_current_lots(c: FPSupport, e: NewOrderEvent):
...
def update_profile_lots(c: FPSupport, e: NewOrderEvent):
...
def update_lot_state(funpayautobot: FPSupport, lot: types.LotShortcut, task: int) -> bool:
    """
    Обновляет состояние лота

    :param funpayautobot: объект Кардинала.
    ...
    while attempts:
        try:
            lot_fields = funpayautobot.account.get_lot_fields(lot.id)
            if lot_fields.auto_delivery:
                ...
                funpayautobot.account.save_lot(lot_fields)
...
def update_lots_states(funpayautobot: FPSupport, event: NewOrderEvent):
    if not any([funpayautobot.autorestore_enabled, funpayautobot.autodisable_enabled]):
        return
    curr_profile_tag = funpayautobot.curr_profile_last_tag
    if funpayautobot.last_state_change_tag == curr_profile_tag:
        return
    funpayautobot.last_state_change_tag = curr_profile_tag
    lots = funpayautobot.curr_profile.get_sorted_lots(1)

    deactivated = []
    restored = []
    for lot in funpayautobot.profile.get_sorted_lots(3)[SubCategoryTypes.COMMON].values():
        ...
        # 1 - восстановить
        current_task = 0
        config_obj = get_lot_config_by_name(funpayautobot, lot.description)
        ...
        # Если же лот активен
            ...
            if config_obj:
                ...
                if all((not products_count, funpayautobot.MAIN_CFG["FunPay"].getboolean("autoDisable"),
                        config_obj.get("disableAutoDisable") in ["0", None])):
                    current_task = -1

        if current_task:
            result = update_lot_state(funpayautobot, lot, current_task)
            if result:
                if current_task == -1:
...
<code>{lots}</code>"""
        Thread(target=funpayautobot.telegram.send_notification, args=(text,),
...
def update_profiles_handler(funpayautobot: FPSupport, event: NewOrderEvent | OrdersListChangedEvent, *args):
    """Обновляет информацию о профилях и состояния лотов в отдельном потоке."""
    def f(c: FPSupport, e: NewOrderEvent):
        update_current_lots(c, e)
        update_profile_lots(c, e)
        update_lots_states(c, e)

    if event.runner_tag != funpayautobot.last_profile_refresh_event_tag:
        funpayautobot.last_profile_refresh_event_tag = event.runner_tag
        Thread(target=f, args=(funpayautobot, event), daemon=True).start()

# BIND_TO_ORDER_STATUS_CHANGED
def send_thank_u_message_handler(funpayautobot: FPSupport, event: OrderStatusChangedEvent):
    """
    Отправляет ответное сообщение на подтверждение заказа.
    """
    if not funpayautobot.MAIN_CFG["OrderConfirm"].getboolean("sendReply") or event.order.status is not types.OrderStatuses.CLOSED:
        return

    text = FPManager.format_order_text(funpayautobot.MAIN_CFG["OrderConfirm"]["replyText"], event.order)
    chat = funpayautobot.account.get_chat_by_name(event.order.buyer_username)
    if chat:
        chat_id = chat.id
    else:
    ...
    Thread(target=funpayautobot.send_message, args=(chat_id, text, event.order.buyer_username),
           kwargs={'watermark': funpayautobot.MAIN_CFG["OrderConfirm"].getboolean("watermark")}, daemon=True).start()


def send_order_confirmed_notification_handler(funpayautobot: FPSupport, event: OrderStatusChangedEvent):
    """
    ...
    if chat:
        ...
        chat_id = event.order.chat_id
    Thread(target=funpayautobot.telegram.send_notification,
           args=(
...
def send_bot_started_notification_handler(c: FPSupport, *args):
    """
    Отправляет уведомление о запуске бота в телеграм.
    """
    if c.telegram is None:
        return
    text = _("FunPayAutobot_init", c.account.username, c.account.id,
